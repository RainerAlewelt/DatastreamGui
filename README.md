...
- **Unicast, Multicast, and Broadcast** UDP reception
- **Network interface selection** — pick a specific interface for multicast and broadcast, or use all
- **Stream discovery** — scan for active IENA streams on the network (respects interface selection)
...
- **Layout save/load** — persist and restore workspace arrangements as JSON (with filename prompt)
...
Default (26 variables, a–z, multicast):
```bash
...
Broadcast mode with a specific interface:
```bash
python3 iena_multicast_sender.py --broadcast 192.168.100.255 --iface 192.168.100.5 --vars temp,pressure --config-out bc.xidml
```

The sender generates a `.xidml` metadata file on startup and begins sending IENA packets.
...
2. Choose a tab: **Unicast**, **Multicast**, or **Broadcast**
3. For Multicast/Broadcast: optionally select a specific **Network Interface** from the dropdown
4. Browse for the `.xidml` file generated by the sender
5. Click **Connect** (or **Scan for Streams** on the Multicast tab to discover active streams first)
6. The variable dropdown populates from the XidML — select a variable, choose a display type, and click **Add**
...
--broadcast ADDR    Broadcast address (e.g. 255.255.255.255), overrides --group
--iface IFACE       Source interface IP for multicast/broadcast
...
└── src/main/java/dyn/
    ...
    ├── ConnectionWindow.java     # Connect dialog (unicast/multicast/broadcast + interface selection)
